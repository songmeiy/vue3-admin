...
import { baseMessage, baseLoading } from '@/components/ElementPlus'
...
instance.interceptors.request.use((config) => {
  config.withCredentials = withCredentials // 是否允许携带cookie
  config.headers['Content-Type'] = contentType
  const token = store.state.user.accessToken
  if (token) config.headers[tokenName] = token // 将token添加到请求头
