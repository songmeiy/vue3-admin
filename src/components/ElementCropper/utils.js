...
export function getOrientation(arrayBuffer) {
  ...
  let orientation // 当图像没有正确的Exif信息时忽略范围错误(大部分图片是没有方向值的，只有照片图片才有方向值，如果是普通图片获取不到方向值直接返回1正常正常处理就行)

  try {
    let littleEndian
    let app1Start
    let ifdStart // 仅处理JPEG图像 (start by 0xFFD8)
