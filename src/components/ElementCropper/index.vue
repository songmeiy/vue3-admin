    ...
    <div class="cropper" v-bind="$attrs">
      ...
      >
        <img
          ...
          :src="img"
          alt=""
          :style="{ transform: `translate3d(${originalImage.left}px,${originalImage.top}px,0) rotateZ(${originalImage.rotate}deg) scale(${originalImage.scale})` }"
        >
        <div
          class="cropper__box"
          :style="{
        ...
        >
          <div class="face">
            <img
              :src="img"
              alt=""
              :style="{ transform: `translate3d(${originalImage.left - cropBox.left}px,${originalImage.top - cropBox.top}px,0) rotateZ(${originalImage.rotate}deg) scale(${originalImage.scale})` }"
            >
          </div>
...
export default {
  ...
  emits: ['save', 'cancel', 'success', 'fail'],
  setup(props, { emit }) {
        ...
        img.onload = (e) => {
          ...
          if (orientation === 1 && (!maxImgSize || width <= maxImgSize && height <= maxImgSize)) {
            ...
            if (maxImgSize && (width > maxImgSize || height > maxImgSize)) {
              const scale = Math.min(maxImgSize / width, maxImgSize / height)
              width *= scale
              height *= scale
            }
            // 使用校正过的最终数据
    ...
    const initialState = () => {
      ...
      if (width > stage.value.clientWidth || height > stage.value.clientHeight) {
        // 计算舞台区域占据图片的宽高比例
        const widthScale = stage.value.clientWidth / width
        const heightScale = stage.value.clientHeight / height
        // 取最小比例保证最长边能够显示出来
    ...
    const cropBoxRelativeInfo = () => {
      ...
      const { scale, rotate, width, height, left, top } = state.originalImage
      // 因为图片是居中在舞台中间的，所以上右下左都可能会超过舞台，需要取超过的一半值加上自身x、y得到肉眼看到的坐标值
      // 原图宽超出显示区域以外的中心点(需要这个值换算成真实的起始x)
      const outsideAnchorX = (width * (1 - scale)) / 2
      // 原图高超出显示区域以外的中心点(需要这个值换算成真实的起始y)
      const outsideAnchorY = (height * (1 - scale)) / 2
      // 截图的起始x(相对图片的x)
      ...
      const w = Math.ceil(state.cropBox.width / scale)
      // 截取的高度(原理同上)
      const h = Math.ceil(state.cropBox.height / scale)
    ...
    const setOutputSize = () => {
      // 获取裁剪框的相对图片信息
      const { x, y, w, h } = cropBoxRelativeInfo()
    ...
    const onTouchMove = (res) => {
      ...
      // 正在缩放的情况下不触发单指操作
      if (touchZoom) return
      const { touches, mouseInfo, originalImageLeft, originalImageTop, cropLeft, cropTop, cropWidth, cropHeight } = touchStartInfo
      // 移动和触摸开始的xy距离
      let disX
      let disY
      if (res.touches) {
      ...
      if (!dragTarget) {
        ...
        // 改变裁剪框
        let left = cropLeft; let top = cropTop; let width = cropWidth; let height = cropHeight
        switch (dragTarget) {
            case 'dot-tl':
     ...
     */
    const onUpload = (file) => {
      return request(props.action, file, props.data, props.headers, props.name, emit)
    }
    /**
     * 取消裁剪
     */
